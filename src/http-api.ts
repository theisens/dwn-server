...
export class HttpApi {
    ...
    function readReplyHandler(res, reply: RecordsReadReply): any {
      if (reply.status.code === 200) {
        if (reply?.entry?.data) {
          ...
          res.setHeader(
            "content-type",
            reply.entry.recordsWrite.descriptor.dataFormat
          );
    ...
    this.#api.get("/:did/read/protocols/:protocol/*", async (req, res) => {
      ...
      try {
        ...
        for (const param in req.query) {
          ...
          const lastLevelObject = keys.reduce(
        ...
        queryOptions.filter.protocolPath = req.params[0].replace(
          leadTailSlashRegex,
          ""
        );
        ...
        const { entries, status } = await this.dwn.processMessage(
          req.params.did,
          query.message
        );
      ...
      this.#api.get(
        ...
        async (_req: Request, res: Response) => {
          const proofOfWorkChallenge =
            this.registrationManager.getProofOfWorkChallenge();
    ...
    if (this.#config.termsOfServiceFilePath !== undefined) {
      this.#api.get(
        ...
        (_req: Request, res: Response) =>
          res.send(this.registrationManager.getTermsOfService())
      );
    }
  ...
  #setupWeb5ConnectServerRoutes(): void {
      ...
      if (req.body?.request?.request_uri) {
        return res.status(400).json({
          ...
          status: {
            code: 400,
            message:
              "Bad Request: 'request_uri' parameter is not allowed in PAR"
          }
        });
      }
    ...
    this.#api.get("/connect/authorize/:requestId.jwt", async (req, res) => {
      log.info(
        `Retrieving Web5 Connect Request object of ID: ${req.params.requestId}...`
      );

      // Look up the request object based on the requestId.
      const requestObjectJwt =
        await this.web5ConnectServer.getWeb5ConnectRequest(
    ...
    this.#api.post("/connect/callback", async (req, res) => {
      log.info(
        "Storing Identity Provider (wallet) pushed response with ID token..."
      );
    ...
    this.#api.get("/connect/token/:state.jwt", async (req, res) => {
      ...
      const idToken = await this.web5ConnectServer.getWeb5ConnectResponse(
        req.params.state
      );
